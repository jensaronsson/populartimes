...
def get_circle_centers(b1, b2, radius):
    ...
    dist_lat = int(vincenty(geopy.Point(
        sw[0], sw[1]), geopy.Point(ne[0], sw[1])).meters)
    dist_lng = int(vincenty(geopy.Point(
        sw[0], sw[1]), geopy.Point(sw[0], ne[1])).meters)
    ...
    def cover(p_start, n_lat, n_lng, r):
        ...
        for i in range(n_lat):
            for j in range(n_lng):
                ...
                _coords.append(v_north.destination(
                    v_east.destination(point=p_start, bearing=90), bearing=0))
    ...
    sw = vc_radius.destination(
        vc_radius.destination(point=sw, bearing=0), bearing=90)
...
def get_radar(_lat, _lng):
    ...
    if len(radar) > 200:
        logging.warning(
            "more than 200 places in search radius, some data may get lost")
...
def get_detail(place_id):
    ...
    detail_json["populartimes"] = get_popularity_for_day(
        popularity) if popularity is not None else []
...
def get_current_popularity(place_identifier):
    ...
    search_url = "https://www.google.se/search?" + \
        "&".join(k + "=" + str(v) for k, v in params_url.items())
    logging.info("searchterm: " + search_url)
    gcontext = ssl.SSLContext(ssl.PROTOCOL_TLSv1)
...
def get_current_popular_times(api_key, place_id):
    ...
    place_identifier = "{} {}".format(
        detail["name"], detail["formatted_address"])
    rating, rating_n, popularity, current_popularity = get_current_popularity(
        place_identifier)
